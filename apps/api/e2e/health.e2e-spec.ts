...
describe('Application Health E2E Tests', () => {
  ...
  describe('Application Status', () => {
    it('should be able to connect to the API server', async () => {
      ...
      const response = await request.get('/posts').expect((res) => {
    ...
    it('should handle CORS properly', async () => {
      ...
      // CORS might not be configured, so we expect 404 or CORS success
  ...
  describe('Error Handling', () => {
    it('should return 404 for non-existent routes', async () => {
      await request.get('/non-existent-route').expect(404);
    });
    ...
    it('should handle requests with invalid content-type', async () => {
      await request
        ...
        .send('invalid data')
        .expect((res) => {
          // Should return 400, 415, or 401 (if auth is required first)
          expect([400, 401, 415].includes(res.status)).toBe(true);
        });
    });
  });
  ...
  describe('Rate Limiting & Security', () => {
    ...
    it('should reject requests that are too large', async () => {
      const largePayload = 'x'.repeat(100 * 1024 * 1024); // 100MB of data (larger than 50MB limit)

      await request
        .post('/posts')
        .send({ content: largePayload })
        .expect((res) => {
          // Should reject with 413 (Payload Too Large), 400, or timeout
          expect([400, 401, 413, 500].includes(res.status)).toBe(true);
        });
    });
  });
